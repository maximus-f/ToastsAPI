package me.perotin.adapter.v1_21_R1;
...
/**
 * Implementation of the ToastAdapter interface for Minecraft 1.21.4 (v1_21_R1).
 * This class uses Bukkit's Unsafe API to create and register advancements for toast notifications.
 */
public class ToastAdapter_v1_21_R1 implements ToastAdapter {
        ...
        ToastImpl toast = new ToastImpl(icon, message, style) {
            ...
            protected void createAdvancement() {
                try {
                    ...
                    Bukkit.getLogger().info("Advancement JSON: " + json);

                    // Use Bukkit's Unsafe API to load the advancement
                    Bukkit.getUnsafe().loadAdvancement(getKey(), json);

                } catch (Exception e) {
                    Bukkit.getLogger().warning("Error creating advancement: " + e.getMessage());
                    e.printStackTrace();
                }
            }
        };
